...
%TODO add \cite{ComparisonOfMethods} 
...
The following methods have been used. %TODO Remove this
\par
...
\par 
%TODO No our paper does compare few methods. This part is not necessary.
...
%TODO Make reference to ComparisonOfMethods and write sentence that that is comprehensive comparance of methods, 
% so here would not be described others algoritms.
...
%TODO Say that we compare our algorithm with Kahan which is classic and 
% cascade acc which is best solution by performance and precision in paper ComparisonOfMethods(make \cite), 
% so we do not compare to other algorithms.
\par
...
\par 
%TODO this is already described in intro and here should be done with real numbers
%TODO That means one 1.0,  $2^24 * \epsilon$, where $b^{-(p-1)}/2$ as defined by Demmel ...
...
%TODO it is not 3 it is $2.0 + 4\epsilon$, where $b^{-(p-1)}/2$ as defined by Demmel
...
For example, there would be a[33] instead of a[5]. %TODO Bad sencence, it is not connected to previous.
\par 
...
\par
%TODO Implement double precision version.
